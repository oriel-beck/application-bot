...
- `bug-report-util.ts` — bug-report embed from applied topic tags (`Website`, `App`, `BDL`, `BDFD Wiki`, `Flowcharts`; warn if none or more than one). App asks for phone model, OS version, and app version (Settings → info icon). All topic tags ask for videos/photos. Buttons: Resolved (green) and Not a bug (red) via `forum:bug:{tagId}`.
