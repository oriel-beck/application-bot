# Legal

| Document         | Markdown                                     | Public HTML (GitHub Pages)                       |
| ---------------- | -------------------------------------------- | ------------------------------------------------ |
| Privacy Policy   | [privacy-policy.md](./privacy-policy.md)     | [privacy-policy.html](./privacy-policy.html)     |
| Terms of Service | [terms-of-service.md](./terms-of-service.md) | [terms-of-service.html](./terms-of-service.html) |
| Index            | —                                            | [index.html](./index.html)                       |

Discord’s portal often blocks `github.com` blob/raw links. After enabling Pages on the `/docs` folder, use:

- Home: `https://oriel-beck.github.io/application-bot/` ([docs/index.html](../index.html))
- `https://oriel-beck.github.io/application-bot/legal/privacy-policy.html`
- `https://oriel-beck.github.io/application-bot/legal/terms-of-service.html`

These describe the private Application Bot operated for the Bot Designer for Discord community. They are practical policies based on current bot behavior, not formal legal advice.
