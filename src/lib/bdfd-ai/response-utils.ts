import { AttachmentBuilder, Colors, EmbedBuilder, type InteractionEditReplyOptions } from 'discord.js';
...
export function buildThinkingEmbed(status: string): EmbedBuilder {
  return new EmbedBuilder().setColor(Colors.Grey).setTitle('BDFD AI').setDescription(status);
}

export function buildAiEmbed(description: string): EmbedBuilder {
  return new EmbedBuilder().setColor(Colors.Blurple).setTitle('BDFD AI').setDescription(description);
}
...
export function buildUnavailableEmbed(): EmbedBuilder {
  return buildAiEmbed('The AI assistant is currently unavailable. Please wait for a staff member to help you.');
}
...
export function buildDisabledEmbed(): EmbedBuilder {
  return buildAiEmbed('The AI assistant is currently turned off. Please wait for a staff member to help you.');
}
...
export function formatAiPayload(text: string): AiReplyPayload {
  ...
  const previewBody = text.slice(0, AI_EMBED_PREVIEW_MAX) + (text.length > AI_EMBED_PREVIEW_MAX ? '\n\n…' : '');
...
export function startThinkingAnimation(surface: ThinkingSurface, intervalMs = 2500): { stop: () => void } {
  let index = 0;
  const interval = setInterval(() => {
    index = (index + 1) % THINKING_STATUSES.length;
    surface.edit({ embeds: [buildThinkingEmbed(THINKING_STATUSES[index]!)] }).catch(() => null);
  }, intervalMs);
