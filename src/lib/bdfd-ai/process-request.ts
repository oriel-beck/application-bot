...
async function replyInteraction(interaction: Command.ChatInputCommandInteraction, options: AiReplyPayload) {
...
async function createThinkingSurface(source: AiRequestSource): Promise<EditableMessage | null> {
  ...
  await source.interaction.editReply({ embeds: [buildThinkingEmbed(THINKING_STATUSES[0]!)] }).catch(() => null);
...
export async function processAiRequest(options: ProcessAiRequestOptions): Promise<void> {
  const { container, channelId, userText, source, ignoreAiEnabled, skipChannelTracking } = options;
  ...
  if (!skipChannelTracking) {
    ...
    if (!gate.ok) {
      const embed = gate.reason === 'busy' ? buildBusyEmbed() : buildRateLimitEmbed(gate.limit);
  ...
  try {
    const history = skipChannelTracking ? [] : await container.aiConversation.getHistory(channelId);
    ...
    animation.stop();
    const embed = err instanceof BdscriptValidationError ? buildValidationFailedEmbed() : buildUnavailableEmbed();
