...
export async function loadAiConversationListRows(
  ...
  return Promise.all(
    channels.map(async (ch) => {
      const [usage, limit] = await Promise.all([getUsage(ch.channelId), getLimit(ch.channelId)]);
      return {
...
export async function buildAiConversationListMessage(
  ...
  if (!rows.length) {
    container.addTextDisplayComponents(new TextDisplayBuilder().setContent('No AI conversations found.'));
  } else {
    for (const item of rows) {
      container
        .addTextDisplayComponents(
          new TextDisplayBuilder().setContent(`**${item.usage}/${item.limit}** · <#${item.channelId}>`)
        )
        .addActionRowComponents(
          new ActionRowBuilder<ButtonBuilder>().addComponents(
            new ButtonBuilder()
              .setCustomId(`${AiConversationCustomIDs.buttons.reset}:${item.channelId}:${safePage}`)
              .setLabel('Reset usage')
              .setStyle(ButtonStyle.Primary),
            new ButtonBuilder()
              .setCustomId(`${AiConversationCustomIDs.buttons.delete}:${item.channelId}:${safePage}`)
              .setLabel('Delete')
