...
export function generateApplicationListEmbed(
  ...
  return [new EmbedBuilder().setTitle('Application list').setDescription(`${base}${extra}`).setColor(Colors.Aqua)];
}
...
export function generateApplicationListComponents(
  ...
  for (let i = 0; i < selectRowCount; i++) {
    const chunk = pageSlice.slice(i * OPTIONS_PER_SELECT, i * OPTIONS_PER_SELECT + OPTIONS_PER_SELECT);
    if (!chunk.length) break;
    rows.push(new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(generateStringSelectMenu(i, chunk)));
  }
