...
const mapQuestionsAndAnswersToFields = (questions: string[], answers: string[], page: number): APIEmbedField[] =>
  questions.map((q, i) => ({ name: `Q${questionNumber(i, page)}) ${q}`, value: answers[i] || 'N/A' }));
