...
export class SlashCommand extends Command {
  public async chatInputRun(interaction: Command.ChatInputCommandInteraction) {
    ...
    const create = await this.container.applications
      .create(interaction.user.id, questions, msg.id)
      .catch((err) => console.log(err));

    if (!create || !create.at(0)) {
      msg.delete().catch(() => null);
      return interaction.editReply(
        'Failed to create application, please try again later, if this error repeats open a ticket.'
      );
    }

    const edit = await msg
      .edit({
