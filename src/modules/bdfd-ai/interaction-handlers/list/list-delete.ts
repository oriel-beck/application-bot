...
export class AiConversationListDeleteHandler extends InteractionHandler {
  public async run(interaction: ButtonInteraction) {
    ...
    const parsed = parseAiConversationChannelPageCustomId(interaction.customId, AiConversationCustomIDs.buttons.delete);
    if (!parsed) return;

    await this.container.aiConversation.delete(parsed.channelId);
  ...
  public parse(interaction: ButtonInteraction) {
    if (!parseAiConversationChannelPageCustomId(interaction.customId, AiConversationCustomIDs.buttons.delete)) {
      return this.none();
    }
    return this.some();
  }
