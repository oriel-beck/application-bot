...
export function generatePostHelpEmbed(appliedTags: string[]) {
  const mainTags = [
    ...
    container.config.support_tags.wiki_error,
    container.config.support_tags.question
  ] as const;
  const complex = container.config.support_tags.complex;

  let warning: string | undefined;
  const amountOfMainTags = mainTags.reduce(
    (current, value) => (appliedTags.includes(value) ? current + 1 : current),
    0
  );
  if (!amountOfMainTags)
    warning =
      'You cannot have a post without the tags `code error`, `wiki error` or `question`, please apply the appropriate tag to your post';
  if (amountOfMainTags > 1)
    warning =
      'You cannot have a post with multiple topic tags, please use `code error`, `wiki error` or `question` alone, you can add the `complex` tag alongside it if you feel like your issue is complex.';

  const embed = new EmbedBuilder().setTitle('BDFD Post support').setColor(Colors.Blurple);
  if (warning) embed.setDescription(`⚠️ ${warning}`);
  if (appliedTags.includes(mainTags[0]))
    embed.addFields({
      name: 'Code error',
      value:
        "` ● ` Please send your code as a code block (\\`\\`\\`), not as a screenshot.\n` ● ` Please provide the exact error you're facing.\n` ● ` If your bot tells you where the error is, please make sure to check there before making a post."
    });
  if (appliedTags.includes(mainTags[1]))
    embed.addFields({
      name: 'Wiki error',
      value:
  ...
  if (appliedTags.includes(mainTags[2]))
    embed.addFields({
      name: 'Question',
      value:
        '` ● ` Make sure your question is easy to read.\n` ● ` Check <#567724334278508554> to see if someone already answered your question.'
    });
  if (appliedTags.includes(complex))
  ...
  embed.addFields({
    name: 'AI assistant',
    value:
      '` ● ` Use **`/ai`** or reply to **this message** for wiki-powered help. Conversation history is saved for follow-ups.'
  });
  embed.setFooter({
